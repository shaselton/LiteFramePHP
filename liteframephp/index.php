 ...
 *
 * http://www.LiteFramePHP.com
 * @package    LiteFramePHP
 * @copyright  Copyright (c) 2009 through 2010, Mahdi Pedramrazi, Scott Haselton
...
class  LiteFramePHP {
    ...
    public  function Action(){
       ...
       $path = $this->fileSystemPath.$this->actionFolder.self::$yAction['_LiteFramePHP_']['ACTION'].".php";
       if(!file_exists ( $path )){ die ("File <b>".$path." </b> <br />Not Found or File doesn't have the permission to open");      }
    
       require_once($path);
       
       // NOTE:  if you want to change the class name of the action, or want to include a level of inheritance throughout the project with a specific name, change this line.
       $LiteFramePHPAction = new LiteFramePHPAction;
       
       
       $this->LiteFramePHPVars = &self::$yAction['_LiteFramePHP_']['VARS'];
    ...
    public function LoadTemplate(){
    	...
    	if($this->javaScriptActionInfo){ $this->LiteFramePHPVars['javaScriptActionInfo'] = json_encode($this->LiteFramePHPVars); }
...
*****************************************/
     ...
     * @return null
     */
    public function UpdateyActionVariables(){ self::$yAction['_LiteFramePHP_']['VARS'] = get_object_vars($this);  }
     ...
     */
    public  function GetTemplateName(){    return self::$yAction['_LiteFramePHP_']['VARS']['templateAction'];  }           	
    ...
    public function GetDisplay(){
        ...
        switch($this->actionType){
            case "AJAX" : 
              $display = self::$yAction['_LiteFramePHP_']['VARS']['actionTrigger'];
              break;
    ...
    public function UpdateFoldersPaths(){
        ...
        $this->LiteFramePHPVars['fileSystemPaths']['templateActionPath'] = 
        $this->fileSystemPaths['templatePath'].$this->actionFolder.$this->templateEngineFolder;
 
        $this->fileSystemPaths['templateIncludePath'] = 
        $this->LiteFramePHPVars['fileSystemPaths']['templateIncludePath'] = 
        $this->fileSystemPaths['templatePath'].$this->includeFolder.$this->templateEngineFolder;
        
        $this->fileSystemPaths['templateLayoutPath'] = 
        $this->LiteFramePHPVars['fileSystemPaths']['templateLayoutPath'] = 
        $this->fileSystemPaths['templatePath'].$this->layoutFolder.$this->templateEngineFolder;        

        $this->fileSystemPaths['javascriptPath'] =
        $this->LiteFramePHPVars['fileSystemPaths']['javascriptPath'] = $this->fileSystemPath.$this->javascriptFolder.$this->templateEngineFolder;
        
        $this->fileSystemPaths['javascriptModulePath'] =
        $this->LiteFramePHPVars['fileSystemPaths']['javascriptModulePath'] = $this->fileSystemPath.$this->javascriptFolder.$this->templateEngineFolder.$this->javascriptModuleFolder;
        
        
        $this->fileSystemPaths['stylePath'] =
        $this->LiteFramePHPVars['fileSystemPaths']['stylePath'] 	  = $this->fileSystemPath.$this->styleFolder.$this->templateEngineFolder.$this->templateColorName;
        
        $this->LiteFramePHPVars['actionFolder'] = $this->actionFolder.$this->templateEngineFolder;
        $this->LiteFramePHPVars['includeFolder'] = $this->includeFolder.$this->templateEngineFolder;
        $this->LiteFramePHPVars['layoutFolder'] = $this->layoutFolder.$this->templateEngineFolder;
        
        $this->LiteFramePHPVars['stylePath'] = $this->applicationPath.$this->styleFolder.$this->templateEngineFolder.$this->templateColorName;
        $this->LiteFramePHPVars['imagePath'] = $this->applicationPath.$this->styleFolder.$this->templateEngineFolder.$this->templateColorName.$this->imagesFolder;
        $this->LiteFramePHPVars['javascriptPath'] = $this->applicationPath.$this->javascriptFolder.$this->templateEngineFolder;
        $this->LiteFramePHPVars['javascriptModulePath'] = $this->applicationPath.$this->javascriptFolder.$this->templateEngineFolder.$this->javascriptModuleFolder;
        if(!isset($this->LiteFramePHPVars['javascriptLibraryPath'])){
        	$this->LiteFramePHPVars['javascriptLibraryPath'] = $this->applicationPath.$this->javascriptLibraryFolder;
        }
        if(!isset($this->LiteFramePHPVars['styleLibraryPath'])){
        	$this->LiteFramePHPVars['styleLibraryPath'] = $this->applicationPath.$this->styleLibraryFolder;
        }
        $this->LiteFramePHPVars['javaScriptActionInfo'] = $this->javaScriptActionInfo;
        
        $this->fileSystemPaths['settingsPath'] = 
        $this->LiteFramePHPVars['fileSystemPaths']['settingsPath'] = 
        $this->fileSystemPaths['templatePath'].$this->settingsFolder;
		
        if($this->actionType == "INCLUDE_JSON"){
        	$data = self::$yAction; 
        	unset($data['_LiteFramePHP_']);
        	$this->LiteFramePHPVars['yActionJson']  =  $data;    
		}
    ...
    public function MergeActionTemplatesWithFramework(){
         ...
         $this->templateLayoutName = $this->LiteFramePHPVars['templateLayoutName'];
         $this->actionType = $this->LiteFramePHPVars['actionType'];
    ...
    public function SetActionTrigger(){
        ...
        $this->actionTrigger = 
        $this->LiteFramePHPVars['actionTrigger'] = 
    ...
    public  function SetAction(){     
	   ...
	   $request = self::$yAction['_LiteFramePHP_']['REQUEST'];
	   if(  $this->ActionExist($request) ){
	       
	     $this->action = self::$yAction['_LiteFramePHP_']['ACTION'] = $request[$this->actionQuerystring];                   
	   
       }else{  self::$yAction['_LiteFramePHP_']['ACTION'] = $this->action;  }
     ...
     */
    public function ActionExist($request){
	...
	public function CreateRelatedFiles(){
		...
		$jsFile = $this->fileSystemPaths['javascriptPath'].self::$yAction['_LiteFramePHP_']['ACTION'].".js";
		if(file_exists($jsFile)){
			$this->LiteFramePHPVars['javascriptIncludes'][] = self::$yAction['_LiteFramePHP_']['ACTION'].".js"; 
			$this->LiteFramePHPVars['javascriptIncludes'] = array_unique($this->LiteFramePHPVars['javascriptIncludes']);
		}

	    
		$cssFile = $this->fileSystemPaths['stylePath'].self::$yAction['_LiteFramePHP_']['ACTION'].".css";
		if(file_exists($cssFile)){
			$this->LiteFramePHPVars['styleIncludes'][] = self::$yAction['_LiteFramePHP_']['ACTION'].".css"; 
			$this->LiteFramePHPVars['styleIncludes'] = array_unique($this->LiteFramePHPVars['styleIncludes']);
		}
...
******************************************************/     
	 ...
	 * @return null
	 */
	public static function SetTemplateAction($templateAction){  self::$yAction['_LiteFramePHP_']['VARS']['templateAction'] = $templateAction;  }
     ...
     * @return null
	 */
	public function SetTemplateFolderName($templateFolder){  
   	
        if(self::$yAction['_LiteFramePHP_']['VARS']['templateEngine'] == 1 || $this->templateEngine === true){
          
            $this->templateEngineFolder = self::$yAction['_LiteFramePHP_']['VARS']['templateEngineFolder'] = $templateFolder."/";
    ...
    private static function IncludeFiles($args, $type){
    	...
    	$includes = (isset(self::$yAction['_LiteFramePHP_']['VARS'][$type])) 
           ? self::$yAction['_LiteFramePHP_']['VARS'][$type]
           : array();
        if(count($args) == 1 && is_array($args[0])) {  $args = $args[0]; } 
    	self::$yAction['_LiteFramePHP_']['VARS'][$type] = array_unique(array_merge($includes,$args));
     ...
     * @return null
     */
    public static function InlineStyle($code){  self::$yAction['_LiteFramePHP_']['VARS']['styleInline'] = $code; }     
     ...
     * @return null
     */
    public static function InlineJavascript($code){  self::$yAction['_LiteFramePHP_']['VARS']['javascriptInline'] = $code; } 
	...
	public function SetTemplateColorName($tempalteFolder){  
        ...
        if( (self::$yAction['_LiteFramePHP_']['VARS']['templateEngine'] == 1 && !empty(self::$yAction['_LiteFramePHP_']['VARS']['templateEngineFolder']))
           ||	
           ($this->templateEngine === true && !empty($this->templateEngineFolder))
        ){
          
            $this->templateColorName = self::$yAction['_LiteFramePHP_']['VARS']['templateColorName'] = $tempalteFolder."/";
	...
	public function SetTemplateLayoutName($layout){ 
		...
		$this->templateLayoutName = self::$yAction['_LiteFramePHP_']['VARS']['templateLayoutName'] = $layout; 
     ...
     * @return null
     */
    public static function AJAXLayout(){ self::$yAction['_LiteFramePHP_']['VARS']['actionType'] = "AJAX";  }
     ...
     * @return null
     */
    public static function JSONLayout(){ self::$yAction['_LiteFramePHP_']['VARS']['actionType'] = "JSON";  }    
     ...
     * @return null
     */
    public static function NoTemplateLayout(){ self::$yAction['_LiteFramePHP_']['VARS']['actionType'] = "None";  }    
     ...
     * @return null
     */
    public static function IncludeJson(){ self::$yAction['_LiteFramePHP_']['VARS']['actionType'] = "INCLUDE_JSON";  }  
    ...
    public function SetTemplateEngineFolder(){
        ...
        $this->templateEngineFolder = self::$yAction['_LiteFramePHP_']['VARS']['templateEngineFolder'];    
    ...
    public function SetTemplateEngineColorName(){
        ...
        $this->templateColorName = self::$yAction['_LiteFramePHP_']['VARS']['templateColorName']; 
    ...
    public static function FetchGetVariable(){  return self::$yAction['_LiteFramePHP_']['GET']; }
    
    

    
    public static function FetchPostVariable(){ return self::$yAction['_LiteFramePHP_']['POST']; }

    
    
    
    public static function FetchRequestVariable(){ return self::$yAction['_LiteFramePHP_']['REQUEST']; } 
    
    
    
    
    public static function GetFileSystemPath(){ return LiteFramePHP::$yAction['_LiteFramePHP_']['VARS']['fileSystemPath']; }
    public static function GetApplicationPath(){ return LiteFramePHP::$yAction['_LiteFramePHP_']['VARS']['applicationPath']; }
    public static function GetJavascriptPath(){ return LiteFramePHP::$yAction['_LiteFramePHP_']['VARS']['LiteFramePHPVars']['javascriptPath']; }
    public static function GetJavascriptModulePath(){ return LiteFramePHP::$yAction['_LiteFramePHP_']['VARS']['LiteFramePHPVars']['javascriptModulePath']; }
    public static function GetStylePath(){ return LiteFramePHP::$yAction['_LiteFramePHP_']['VARS']['LiteFramePHPVars']['stylePath']; }
    public static function GetImagePath(){ return LiteFramePHP::$yAction['_LiteFramePHP_']['VARS']['LiteFramePHPVars']['imagePath']; }
    public static function GetJavascriptLibraryPath(){ return LiteFramePHP::$yAction['_LiteFramePHP_']['VARS']['LiteFramePHPVars']['javascriptLibraryPath']; }
	public static function GetStyleLibraryPath(){ return LiteFramePHP::$yAction['_LiteFramePHP_']['VARS']['LiteFramePHPVars']['styleLibraryPath']; }
	public static function GetTemplateIncludePath(){ return LiteFramePHP::$yAction['_LiteFramePHP_']['VARS']['fileSystemPaths']['templateIncludePath']; }
	public static function GetTemplateActionPath(){ return LiteFramePHP::$yAction['_LiteFramePHP_']['VARS']['fileSystemPaths']['templateActionPath']; }
	public static function GetTemplateLayoutPath(){ return LiteFramePHP::$yAction['_LiteFramePHP_']['VARS']['fileSystemPaths']['templateLayoutPath']; }
	public static function GetTemplateSettingsPath(){ return LiteFramePHP::$yAction['_YOTTA_']['VARS']['fileSystemPaths']['settingsPath']; }
...
*****************************************/  
    ...
    /**
     * Sanitize aims to sanitize all of the variables that might be in a POST or GET request.  This is done automatically, but you can
     * still have access to this function to clean further vars if needed.
     * 
     * @access public
     * @param	string | array	$input can be an array or single string of values that one hopes to have sanitized
     * @return	string | array	$ouput will be the matching type that was passed in with the vars cleaned.
     */
